...
import { Image as ImageIcon, UploadCloud, FileText, Download, Share2, X } from 'lucide-react';
      ...
      <main className="flex-grow container mx-auto px-2 sm:px-4 py-6 sm:py-8 grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 mt-12 sm:mt-16">
        ...
        <aside className="lg:col-span-3 bg-gray-800 p-4 sm:p-6 rounded-xl shadow-2xl flex flex-col space-y-6 h-fit lg:sticky lg:top-36">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white font-bold py-3 rounded-lg shadow-xl hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-105 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? 'AI generating...' : 'Generate renderings'}
          </button>
        </aside>
        {/* 右侧：说明区和渲染结果 */}
